...
def display_call_interface():
    phone_number = st.text_input("Phone Number (format: +1XXXXXXXXXX)", value=os.getenv("YOUR_NUMBER") or "")
    name = st.text_input("Enter your name")
    data = {"name": name, "phone_number": phone_number}

    # Automatically save whenever input changes
...
with st.sidebar:
    ...
    phone_number, name = display_call_interface()
    initial_message = f"Hello {name}, I'm Riya from Shivalik Group. I hope I'm not disturbing you. Would this be a good time for a quick 2-minute conversation?" if name else "Hello, I'm Riya from Shivalik Group. I hope I'm not disturbing you. Would this be a good time for a quick 2-minute conversation?"
    
    # Removed system_message and initial_message text areas from frontend
